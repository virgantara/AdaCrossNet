...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='dgcnn', metavar='N',
                        choices=['dgcnn','pointnet','pointnet2'],
                        help='Model to use, [dgcnn,pointnet,pointnet2]')
...
def train(args, io):
    wandb.init(project="PartsegAdaCrossNetBackbone", name=args.exp_name)
    ...
    if args.model == 'dgcnn':
        ...
        model = PointNet2_partseg(args, seg_num_all).to(device)
    else:
...
def test(args, io):
    test_loader = DataLoader(ShapeNetPart(partition='test', num_points=args.num_points, class_choice=args.class_choice, class_test=args.class_test),
...
if __name__ == "__main__":
    ...
    if not args.eval:
        ...
        # for c in ['airplane', 'bag', 'cap', 'car', 'chair',
        #                          'earphone', 'guitar', 'knife', 'lamp', 'laptop',
        #                          'motor', 'mug', 'pistol', 'rocket', 'skateboard', 'table']:
        #     args.class_test = c
        #     print(c)
