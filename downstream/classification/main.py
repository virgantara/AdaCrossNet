...
from data import ModelNet40, ModelNet40Subset
...
def parse_args():
    ...
    parser.add_argument('--normalize_input', type=int, default=0,
                        help='whether to normalize input data to a unit space')
    parser.add_argument('--percent_train', type=float, default=None,
                        help='only use part of training data')
...
def train(args, io):
    if args.percent_train:
        ...
        test_loader = DataLoader(ModelNet40Subset(partition='test', num_points=args.num_points, normalize=args.normalize_input), num_workers=8,
                             batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    else:
        train_loader = DataLoader(ModelNet40(partition='train', num_points=args.num_points, normalize=args.normalize_input), num_workers=8,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(ModelNet40(partition='test', num_points=args.num_points, normalize=args.normalize_input), num_workers=8,
                             batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    ...
    for epoch in range(args.epochs):
        ...
        print("learning rate:", opt.state_dict()['param_groups'][0]['lr'])
        ####################
        # Train
        ####################
        train_loss = 0.0
